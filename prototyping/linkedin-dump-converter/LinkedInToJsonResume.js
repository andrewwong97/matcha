class LinkedInToJsonResume {
    ...
    processProfile(source) {
        ...
        this._extend(this.target.basics, {
            ...
            phone: source.phoneNumbers && source.phoneNumbers._total
                ? source.phoneNumbers.values[0].phoneNumber
                : '',
            website: '',
            ...
            location: {
                ...
                city: source.location
                    ...
                    : '',
                countryCode: source.location
                    ? source.location.country.code.toUpperCase()
                    : '',
                region: ''
            },
            profiles: []
    ...
    processPosition(source) {
        function processPosition(position) {
            let object = {
                ...
                website: '',
                startDate: `${position.startDate.year}-${position.startDate.month < 10
                    ? '0'
                    : ''}${position.startDate.month}-01`,
                summary: position.description,
                highlights: []
            };

            if (position.endDate) {
                object.endDate = `${position.endDate.year}-${position.endDate.month < 10
                    ? '0'
                    : ''}${position.endDate.month}-01`;
            }
    ...
    processSkills(skills) {
        this.target.skills = skills.map(skill => ({name: skill, level: '', keywords: []}));
    }
    ...
    processReferences(source) {
        this.target.references = source.map(reference => ({name: `${reference.recommenderFirstName} ${reference.recommenderLastName} - ${reference.recommenderCompany}`, reference: reference.recommendationBody}));
    }

    processInterests(source) {
        this.target.interests = source.map(interest => ({name: interest, keywords: []}));
    }
    ...
    processProjects(source) {
        function processProjects(project) {
            ...
            let p = {
                name: project.title,
                startDate: `${project.startDate.year}-${project.startDate.month < 10
                    ? '0'
                    : ''}${project.startDate.month}-01`,
                summary: project.description,
                url: project.url
            };
            if (project.endDate) {
                p.endDate = `${project.endDate.year}-${project.endDate.month < 10
                    ? '0'
                    : ''}${project.endDate.month}-01`;
            }
            return p;
        }
